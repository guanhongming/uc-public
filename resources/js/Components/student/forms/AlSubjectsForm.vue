<!-- resources/js/Components/ProfileSections/AlSubjectsForm.vue -->
<template>
    <!-- The outer container has no border or shadow to blend into the parent card -->
    <div class="al-subjects-form">
        <h2 class="text-2xl font-bold text-gray-800">A Level Subjects & Scores</h2>
        <p class="mt-1 mb-8 text-sm text-gray-600">Enter up to 5 A Level subjects and their official CIE scores.</p>
        ...
        <form @submit.prevent="saveForm" class="space-y-4">
            ...
            <div v-for="(subject, index) in subjects" :key="subject.id"
                 ...
                 :class="{ 'is-incomplete': (subject.subject && !subject.score) || (!subject.subject && subject.score) }">

                <div class="grid grid-cols-1 gap-y-2 gap-x-4 sm:grid-cols-5">
                    <!-- Subject Input -->
                    <div class="sm:col-span-2">
                        <label :for="'al-subject-' + index" class="block text-sm font-medium text-gray-700">
                            A Level Subject {{ index + 1 }}
                        </label>
                        <input
                            type="text"
                            :id="'al-subject-' + index"
                     ...
                     <!-- Score Input -->
                    <div class="sm:col-span-2">
                        <label :for="'al-score-' + index" class="block text-sm font-medium text-gray-700">
                             Official CIE Score
                        </label>
                        <input
                            type="text"
...
// --- HELPERS ---
// Find a field ID from the props based on its label.
const getFieldIdByLabel = (label) => props.sectionFields.find(f => f.label === label)?.id || null;

// --- DYNAMIC FIELD LOGIC ---
const addSubject = () => {
    if (subjects.value.length < MAX_SUBJECTS) {
        const nextIndex = subjects.value.length + 1;
        subjects.value.push({
            id: Date.now(), // Unique key for v-for
            subject: '',
            score: '',
            subjectFieldId: getFieldIdByLabel(`A Level Subject ${nextIndex}`),
            scoreFieldId: getFieldIdByLabel(`Official CIE Score ${5 + nextIndex}`),
        });
    }
};
...
watch(() => props.sectionFields, (newFields) => {
    ...
    for (let i = 1; i <= MAX_SUBJECTS; i++) {
        const subjectFieldId = getFieldIdByLabel(`A Level Subject ${i}`);
        const scoreFieldId = getFieldIdByLabel(`Official CIE Score ${5 + i}`);
    ...
    if (populatedSubjects.length === 0) {
        populatedSubjects.push({
            ...
            subject: '',
            score: '',
            subjectFieldId: getFieldIdByLabel('A Level Subject 1'),
            scoreFieldId: getFieldIdByLabel('Official CIE Score 6'),
        });
    }
...
async function saveForm() {
    ...
    try {
        ...
        // Map our dynamic array back to the ID-based structure
        subjects.value.forEach((item, index) => {
            const subjectFieldId = getFieldIdByLabel(`A Level Subject ${index + 1}`);
            const scoreFieldId = getFieldIdByLabel(`Official CIE Score ${5 + (index + 1)}`);

            if (subjectFieldId) {
                answersSubset[subjectFieldId] = item.subject?.trim() || null;
            }
            if (scoreFieldId) {
                answersSubset[scoreFieldId] = item.score?.trim() || null;
            }
        });

        // NOTE: If subjects are removed, their old values will not be cleared from the DB
        // unless we explicitly send nulls for all 5 possible fields. This can be added if needed.

        await axios.post('/api/profile', { answers: answersSubset });
        emit('saved', answersSubset);

    } catch (error) {
        console.error("Error saving A Level Subjects & Scores:", error);
    } finally {
